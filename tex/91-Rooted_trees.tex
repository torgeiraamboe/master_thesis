...
There is another---more graphical---way of describing the Stasheff identities\index{The Stasheff identities} in a $\A$-algebra\index{$A_\infty$-algebra}, and this is done by \emph{rooted trees}\index{Rooted tree}. A rooted tree is a graph with a fixed node at the bottom (which we call the root), and where every other two nodes are connected by no more than one line. An example is the following graph.
...
Adding more leaves to the central vertex gives all the other $m_i$'s as well. We can let these trees interact in certain ways in order to produce the Stasheff identities\index{The Stasheff identities}. Let's start with $n=1$. This relation is given by $m_1 m_1=0$, which we can describe with the following rooted tree:
...
Since we know that a DG-algebra\index{DG-algebra} can be though of as an $\A$-algebra where $m_n = 0$ for all $n\geq 3$, we can explicitly write out all the relations we have in a DG-algebra using these trees. The $n=1$ relation is again given by
...
which we can recognize as the Leibniz rule\index{The Leibniz rule}. We don't have the $(-1)^{|x|}$ sign here, as it first arises when using the Koszul grading rule\index{The Koszul grading rule} when applying these operations to actual elements. The third and final relation is given by 
